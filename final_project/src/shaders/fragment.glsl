...
// // macros are always defined at compile time not runtime
...
void main() {
  ...
  // fract only returns the decimal places
  // step is used to cut the gradient into 2 parts of solid
  // gl_FragColor = vec4(vec3(step(0.5, fract(uv.y * 10.0))), 1.0);

  // vec3 coords = vNormal;
  // coords.y += uTime / 10.0; // animating
